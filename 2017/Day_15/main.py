...
PART 2: After 5 million pairs, but using this new generator logic, what is the
        judge's final count?
"""
...
class GeneratorDuel:
    ...
    def compare_pair(self, val0: int, val1: int) -> bool:
        """
    ...
    def matching_pairs(self, num_pairs: int) -> int:
        ...
        for _ in range(num_pairs):
            self.iterate()
            pair_count += int(self.compare_pair(self.gen_a, self.gen_b))

        return pair_count

    def detatched_matching_pairs(self, num_pairs: int) -> int:
        """
        Record the values for both generators but do so in a detached
        manner.
        """
        gen_a_vals = []
        gen_b_vals = []
        ...
        # Generate the values
        while len(gen_a_vals) < num_pairs or len(gen_b_vals) < num_pairs:
            self.iterate()

            if self.gen_a % 4 == 0:
                gen_a_vals.append(self.gen_a)

            if self.gen_b % 8 == 0:
                gen_b_vals.append(self.gen_b)

        # Compare the values
        pair_count = 0
        for idx in range(num_pairs):
            pair_count += int(self.compare_pair(gen_a_vals[idx], gen_b_vals[idx]))
...
if __name__ == "__main__":
    print(
        f"Part 1 = {GeneratorDuel(634, 301).matching_pairs(40_000_000)}\n"
        f"Part 2 = {GeneratorDuel(634, 301).detatched_matching_pairs(5_000_000)}\n"
    )
