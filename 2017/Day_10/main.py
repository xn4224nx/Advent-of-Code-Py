...
PART 1: However, you should instead use the standard list size of 256 (with
...
PART 2: Treating your puzzle input as a string of ASCII characters, what is the
...
class KnotHash:
    def __init__(self, max_element: int, lengths_file: str, byte_input: bool = False):
        self.pos = 0
        self.skip_size = 0
        ...
        with open(lengths_file, "r") as fp:
            if self.byte_input:
                self.lengths = [ord(x) for x in fp.read().strip()] + [
                    17,
                    31,
                    73,
                    47,
                ...
                self.lengths = [int(x) for x in fp.read().strip().split(",")]
...
if __name__ == "__main__":
    ...
    print(f"Part 2 = {KnotHash(255, "./data/input.txt", True).calc_digest()}")
