...
class SpinLock:
    ...
    def spin(self):
        """
        Rotate the buffer and then append the current index.
        """
        self.buff.rotate(-self.step)
        self.buff.append(self.idx)
        self.idx += 1

    def post_value(self, total_vals_to_gen: int, fvalue: int) -> int:
        """
        Identify the value that will ultimately be after a specified value after a certain number of numbers have been generated.
        """
        while self.idx <= total_vals_to_gen:
            self.spin()

        # Find the index of the fvalue
        f_idx = self.buff.index(fvalue)

        # Return the value before fvalue
        return self.buff[(f_idx + 1) % len(self.buff)]


if __name__ == "__main__":
    print(
        f"Part 1 = {SpinLock(354).post_value(2017, 2017)}\n"
        f"Part 2 = {SpinLock(354).post_value(50_000_000, 0)}\n"
    )
